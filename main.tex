...
impl<'a> ImportantExcerpt<'a>{
    fn return_part(&self, announcement: &str) -> &str{
        println!("Attenetion please: {}", announcement);
        self.part
    }
}
...
\end{lstlisting}
We don't have to specfiy the lifetime of our return\_part function since, by using the three rules the compiler can figure it out itself. 
If gives a lifetime parameter to each input parameter will get a lifetime (so \&self will get 'a and announcement will get 'b). The second rule doesn't apply, but rule three says that the lifetime of \& self will be given to the output lifetime.


\paragraph*{static lifetime}
\begin{definition}
    \textbf{The static lifetime} means that the reference can live as long as the duration of the program. For example all string literals since they are stored in the code binary.
\end{definition}
\newpage
